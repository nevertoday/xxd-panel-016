---
name: xxd-panel-016
description: "Create XXD Panel 016 artwork from supplied photos in four selectable modes: photo above/design below, photo left/design right, transformed Riso design alone, or a four-device wallpaper pack with either independent compositions or anchor-linked visual continuity. Supports exact 50/50 paired layouts, custom dimensions, and source-aware experimental copy. Use for grain-field Riso transformations and the exact 016 style; never use it for collages."
---

# XXD Panel 016 · 粗粝动态场版画

Turn every supplied photograph into finished artwork. The selected mode may show the source photo above, show it on the left, omit it from one transformed canvas, or expand it into four separately composed device wallpapers while still using it as the sole content source.
...
- Resolve one output mode before generation: `top-bottom` (photo above, design below), `left-right` (photo left, design right), `design-only` (only the transformed design is visible), or `wallpaper-pack` (phone, iPad, desktop, and watch wallpapers). If the user has not already specified a mode, ask the one concise selection question defined in the workflow and wait for the answer; never ask again when the mode is already clear.
...
- Pixel dimensions supplied by the user have highest priority, followed by an explicit canvas ratio or destination, followed by the mode default: 3:4 for `top-bottom`, 3:2 for `left-right`, and 3:4 for `design-only`. `wallpaper-pack` has four device defaults defined below and accepts labeled per-device overrides. An exact paired layout requires an even split-axis dimension.
...
2. Resolve dimensions with this priority: exact pixel size > explicit ratio/destination > mode default. For exact `top-bottom`, height must be even; for exact `left-right`, width must be even. Do not silently alter an odd requested split dimension—ask for one of the adjacent even values. `design-only` accepts any positive whole-pixel dimensions. For `wallpaper-pack`, use phone 1440×3200, iPad 2048×2732 portrait, desktop 3840×2160, and watch 1024×1024 unless the user labels a device-specific override. Unspecified devices retain their defaults; ask which device an unlabeled pack size belongs to rather than applying it arbitrarily.
3. Open a new job boundary. Resolve a fresh per-source-and-mode task-directory name using the output rules below, even when an identical earlier result exists. Confirm the actual user-supplied source set under the source-discovery rules above. If none is accessible, ask the user to attach it or provide its path; do not invent a source photo or search output folders for a substitute.
4. Inspect each current-job source image separately. For a local file that has not been seen, use the available image-viewing tool before generation. Never inspect an earlier output in place of the current source.
5. Read the full prompt matching the user's working language:
...
```bash
# top-bottom: legacy behavior, exact horizontal seam
scripts/compose_panel.py --plan --layout top-bottom --canvas 3:4
scripts/compose_panel.py --source photo.png --design panel.png \
    --out ~/Desktop/xxd-panel-016/IMG_4821-top-bottom/IMG_4821.png --layout top-bottom --canvas 3:4

# left-right: exact vertical seam, exact custom resolution
scripts/compose_panel.py --plan --layout left-right --size 2560x1440
scripts/compose_panel.py --source photo.png --design panel.png \
    --out ~/Desktop/xxd-panel-016/IMG_4821-left-right/IMG_4821-lr.png --layout left-right --size 2560x1440

# design-only: no visible photo and no seam
scripts/compose_panel.py --design panel.png \
    --out ~/Desktop/xxd-panel-016/IMG_4821-design-only/IMG_4821-design.png \
    --layout design-only --size 2048x2048

# wallpaper-pack: four separately generated assets, finalized separately
scripts/compose_panel.py --design phone.png --out ~/Desktop/xxd-panel-016/IMG_4821-wallpaper-pack/IMG_4821-wallpaper-phone.png --layout design-only --size 1440x3200
scripts/compose_panel.py --design ipad.png --out ~/Desktop/xxd-panel-016/IMG_4821-wallpaper-pack/IMG_4821-wallpaper-ipad.png --layout design-only --size 2048x2732
scripts/compose_panel.py --design desktop.png --out ~/Desktop/xxd-panel-016/IMG_4821-wallpaper-pack/IMG_4821-wallpaper-desktop.png --layout design-only --size 3840x2160
scripts/compose_panel.py --design watch.png --out ~/Desktop/xxd-panel-016/IMG_4821-wallpaper-pack/IMG_4821-wallpaper-watch.png --layout design-only --size 1024x1024
```

`--size WIDTHxHEIGHT` has priority over `--canvas` and `--width`. Legacy `--top` and `--bottom` remain aliases for `--source` and `--design`.

**Photographic panel.** In `top-bottom`, it fills the upper half; in `left-right`, it fills the left half. Crop gently or extend only the environment when necessary. `--anchor top|center|bottom|left|right` selects what a residual crop keeps. `design-only` has no photographic panel.

**Transformed design.** Generate it alone, filling the exact design frame reported by `--plan`, with no photograph and no unused placeholder space. Append this resolved mode block:
...
Remove unused optional lines rather than rendering placeholders. Only when the user explicitly requests a text-free result, replace the entire block with `COPY MODE: NONE — render no text or pseudo-text anywhere.`
...
**Finalize.** Paired modes place the two assets into exact equal frames. `design-only` resizes/crops the transformed design to the final dimensions without adding a source panel. The script reports severe cover crops so the asset can be regenerated or extended instead.
...
Save every generated poster under `~/Desktop/xxd-panel-016/`. Create the root and task directory if they do not exist.
...
- The output mode and exact dimensions match the resolved selection. `--size` is reproduced pixel-for-pixel.
...
- Unless the user explicitly requested a text-free result, the transformed design contains the locked title and all 2–4 locked microtext strings, accurately readable, with no substitution, gibberish, extra wording, or invented year. The main title is visually unmistakable and at least three times the microtext scale; it is not reduced to a hairline label. In text-free mode, no letters, numbers, captions, or pseudo-text appear anywhere.
